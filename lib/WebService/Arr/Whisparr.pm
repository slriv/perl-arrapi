...
class WebService::Arr::Whisparr :isa(WebService::Arr::Base) {
    use Carp qw(croak);
...
WebService::Arr::Whisparr - Thin Whisparr API v3 client
...
=head1 SYNOPSIS
  ...
  my $all    = $whisparr->series;
  my $one    = $whisparr->series_by_id(42);
  my $cmd    = $whisparr->command('RescanSeries', seriesId => 42);
  my $queue  = $whisparr->queue(page => 1, pageSize => 20);

=head1 DESCRIPTION

L<WebService::Arr::Whisparr> is a thin wrapper over selected Whisparr API v3
endpoints. Whisparr is a Sonarr fork and shares the same Series/Episode API
structure on port 6969. Transport, auth, and error handling are delegated to
